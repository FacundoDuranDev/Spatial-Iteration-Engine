...
@dataclass
class NeuralConfig:
    """Configuración para procesamiento neural (Style Encoder, Stylizer, coherencia temporal)."""

    enabled: bool = True
    style_encoder_path: str = "onnx_models/style_encoder.onnx"
    stylizer_path: str = "onnx_models/stylizer.onnx"
...
class EngineConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not (1 <= self.fps <= 120):
            errors.append(f"fps debe estar entre 1 y 120, se recibió: {self.fps}")

        # Validar grid_w: mínimo 10, máximo 1000
        if not (10 <= self.grid_w <= 1000):
            errors.append(f"grid_w debe estar entre 10 y 1000, se recibió: {self.grid_w}")

        # Validar grid_h: mínimo 10, máximo 1000
        if not (10 <= self.grid_h <= 1000):
            errors.append(f"grid_h debe estar entre 10 y 1000, se recibió: {self.grid_h}")

        # Validar charset: no debe estar vacío
        if not self.charset or len(self.charset) < 2:
            errors.append(f"charset debe tener al menos 2 caracteres, se recibió: '{self.charset}'")

        # Validar render_mode: solo "ascii" o "raw"
        if self.render_mode not in ("ascii", "raw"):
            errors.append(f"render_mode debe ser 'ascii' o 'raw', se recibió: '{self.render_mode}'")
        ...
        if not (0.1 <= self.contrast <= 5.0):
            errors.append(f"contrast debe estar entre 0.1 y 5.0, se recibió: {self.contrast}")

        # Validar brightness: rango razonable -255 a 255
        if not (-255 <= self.brightness <= 255):
            errors.append(f"brightness debe estar entre -255 y 255, se recibió: {self.brightness}")
        ...
        if not (1 <= self.port <= 65535):
            errors.append(f"port debe estar entre 1 y 65535, se recibió: {self.port}")

        # Validar pkt_size: rango razonable 512-65507 (máximo UDP)
        if not (512 <= self.pkt_size <= 65507):
            errors.append(f"pkt_size debe estar entre 512 y 65507, se recibió: {self.pkt_size}")
        ...
        if self.frame_buffer_size < 0:
            errors.append(f"frame_buffer_size debe ser >= 0, se recibió: {self.frame_buffer_size}")

        # Validar sleep_on_empty: debe ser positivo
        if self.sleep_on_empty <= 0:
            errors.append(f"sleep_on_empty debe ser > 0, se recibió: {self.sleep_on_empty}")

        # Validar parallel_workers: mínimo 0 (deshabilitado) o positivo
        if self.parallel_workers < 0:
            errors.append(f"parallel_workers debe ser >= 0, se recibió: {self.parallel_workers}")
        ...
        if not isinstance(self.plugin_paths, list):
            errors.append(f"plugin_paths debe ser una lista, se recibió: {type(self.plugin_paths)}")
        else:
