...
class TrackingPipeline:
    """Pipeline para ejecutar múltiples trackers."""

    def __init__(self, trackers: Optional[Iterable[BaseTracker]] = None) -> None:
        """
        ...
        """
        self._trackers: List[BaseTracker] = list(trackers) if trackers else []
        self._lock = threading.Lock()

    @property
    def trackers(self) -> List[BaseTracker]:
        """Obtiene la lista de trackers."""
        return self._trackers

    def snapshot(self) -> List[BaseTracker]:
        """Obtiene una snapshot thread-safe de los trackers."""
        with self._lock:
            return list(self._trackers)

    def append(self, tracker: BaseTracker) -> None:
        """Agrega un tracker al pipeline."""
        with self._lock:
            self._trackers.append(tracker)

    def extend(self, trackers: Iterable[BaseTracker]) -> None:
        """Extiende el pipeline con múltiples trackers."""
        with self._lock:
            self._trackers.extend(trackers)

    def insert(self, index: int, tracker: BaseTracker) -> None:
        """Inserta un tracker en una posición específica."""
        with self._lock:
            self._trackers.insert(index, tracker)

    def remove(self, tracker: BaseTracker) -> None:
        """Remueve un tracker del pipeline."""
        with self._lock:
            self._trackers.remove(tracker)

    def pop(self, index: int = -1) -> BaseTracker:
    ...
    def replace(self, trackers: Iterable[BaseTracker]) -> None:
    ...
    @contextmanager
    def locked(self) -> Iterator[List[BaseTracker]]:
